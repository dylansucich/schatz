...
#    change code int OOP and plot one vs. the other 
...
print(dfRegrex)
dfRegrex.to_csv("RNAseq-TCanalysis_BWA.csv")
...
# plt.show()
plt.savefig("ParallelLines_EColi_GOI_2.png")
plt.close() 
...
plt.ylim(0, 300)
plt.tight_layout()
fig.savefig( "DepthvsPositionEcoliTimeSeries_verts_TC_Analysis.png" )
plt.close()

fig, ax = plt.subplots()
plt.title("Dendrogram of GOI over Time")
...
fig.savefig("ClusteredDendrogram_TC_Analysis.png")
plt.close(fig)
...
fig.savefig("Depth_all_heatmap_TC_ANALYSIS.png") # Save the image
...
with open("increased.out", 'w') as file:
    for item in list_increased:
        file.write(str(item) + "\n")

with open("decreased.out", 'w') as file:
    for item in list_decreased:
        file.write(str(item) + "\n")
